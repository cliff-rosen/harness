...
def get_all_docs_from_domain(conn, domain_id):
    cur = conn.cursor()
    cur.execute(
        "SELECT doc_id, domain_id, doc_uri, doc_title, doc_text FROM document WHERE domain_id = %s", (domain_id,))
    rows = cur.fetchall()
...
def update_document_chunk_embedding(conn, doc_chunk_id, embedding):
    ...
    cur = conn.cursor()
    cur.execute("UPDATE document_chunk SET chunk_embedding = %s WHERE doc_chunk_id = %s",
                (json_data, doc_chunk_id,))
    conn.commit()
...
def test_get_chunks_from_ids():
    conn = get_connection()
    res = get_document_chunks_from_ids(
        conn, ['36', '34', '30', '33', '32', '31', '35', '89', '66', '65'])
